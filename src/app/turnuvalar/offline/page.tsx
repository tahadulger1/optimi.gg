...
export default function OfflineTurnuvalarPage() {
  const router = useRouter();
  const searchParams = useSearchParams();

  // URL'den filtreleri al - type her zaman offline olacak
  ...
  const handleFiltersChange = (newFilters: TournamentFiltersType) => {
    // Type'ı her zaman offline tut
    const updatedFilters = { ...newFilters, type: TOURNAMENT_TYPE.OFFLINE };
    ...
    router.push(queryString ? `?${queryString}` : "/turnuvalar/offline", {
      scroll: false,
    });
  };

  // Turnuvaları filtrele - sadece offline olanlar
  const filteredTournaments = React.useMemo(() => {
    let result = mockTournaments.filter(
      (tournament) => tournament.type === TOURNAMENT_TYPE.OFFLINE
    );
    ...
    if (filters.search) {
      ...
      result = result.filter(
        (tournament) =>
          ...
          tournament.location?.venue?.toLowerCase().includes(searchLower)
      );
    }
  ...
  return (
    <main className="min-h-screen bg-optimi-darker">
      ...
      {/* Hero Banner - Offline varyantı */}
      <TournamentHero variant="offline" />
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-muted-foreground mt-1">
                {filteredTournaments.length} offline turnuva bulundu
              </p>
            </div>
          </div>
        ...
        {filteredTournaments.length === 0 && (
          <div className="text-center py-12">
            <p className="text-muted-foreground">
              Şu anda aktif offline turnuva bulunmuyor.
            </p>
          </div>
        )}
      </section>
